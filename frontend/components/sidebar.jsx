...
function Sidebar({ current, onLogout }) {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem('quiz:sidebar-collapsed') === '1'; } catch { return false; }
  });
  ...
  useEffect(() => {
    // Clear the early-paint class set by theme-init.js (only needed for splash positioning)
    document.documentElement.classList.remove('sidebar-collapsed');
    const app = document.querySelector('.app');
    if (app) app.classList.toggle('sidebar-collapsed', collapsed);
    document.body.classList.toggle('sidebar-collapsed', collapsed);
    try { localStorage.setItem('quiz:sidebar-collapsed', collapsed ? '1' : '0'); } catch {}
  }, [collapsed]);
  ...
  const userGradient = `linear-gradient(135deg, oklch(75% 0.16 ${(u.username || '').charCodeAt(0) % 360}), oklch(65% 0.18 ${((u.username || '').charCodeAt(0) + 60) % 360}))`;
  const userInitials = (u.name || u.username || '?').split(' ').map(w => w[0]).join('');

  return (
    <aside className={`sidebar ${collapsed ? 'sidebar--collapsed' : ''}`} {...screenLabel('Sidebar')}>
      ...
      <button
        ...
        style={{ marginBottom: 8, width: '100%', justifyContent: collapsed ? 'center' : 'flex-start', padding: collapsed ? '9px' : '9px 12px', fontSize: 13 }}
        onClick={() => nav('editor', { newQuiz: 1 })}
        title={collapsed ? t('nav.new_quiz') : undefined}
      >
        <Icon name="plus" size={15} />
        {!collapsed && t('nav.new_quiz')}
      </button>
      ...
      {!collapsed && <div className="sidebar__section-label">{t('nav.workspace')}</div>}
      ...
      {collapsed ? (
        <button
          ...
          style={{ width: '100%', justifyContent: 'center', padding: '8px 0', borderRadius: 'var(--r-md)', marginBottom: 8 }}
          onClick={() => nav('settings')}
          title={`${u.credits.toLocaleString()} ${t('nav.credits')}`}
        >
          <div style={{
      ...
      {/* Bottom divider */}
      <div style={{ height: 1, background: 'var(--border)', margin: '4px 2px 6px', width: '100%' }} />
      ...
      {collapsed ? (
        <button
          className="btn btn--ghost"
          style={{ width: '100%', justifyContent: 'center', padding: '8px 0', borderRadius: 'var(--r-md)' }}
          onClick={() => nav('settings')}
          title={u.name || u.username}
        >
          <div className="avatar" style={{ background: userGradient }}>{userInitials}</div>
        </button>
      ) : (
